...
export class AppRunnerService extends Construct {
  constructor(scope: Construct, id: string, props: AppStackProps) {
        ...
        AllowGetSecretValue: new PolicyDocument({
          statements: [
            new PolicyStatement({
              ...
              resources: [
                `arn:aws:secretsmanager:${(scope as Stack).region}:${
                  (scope as Stack).account
                }:secret:${props.secretDbUrl.secretName}-*`,
              ],
            }),
          ],
        }),
    ...
    service.addSecret(
      'DATABASE_URL',
      SecretAppRunner.fromSecretsManager(props.secretDbUrl),
    )

    // トレースの有効化
    const cfnObservabilityConfig = new CfnObservabilityConfiguration(
      this,
      'SampleAppRunnerObservConfig',
      {
